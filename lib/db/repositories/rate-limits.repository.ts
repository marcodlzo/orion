 ...
 * a sustained flood cannot overflow a 32-bit column and wrap to a negative
 * number, which would silently re-open the limit.
 */
  ...
  const { rows } = await run<{ hits: number }>(
    ...
    `INSERT INTO rate_limit_counters (bucket, window_start, hits)
     ...
     ON CONFLICT (bucket, window_start) DO UPDATE
       SET hits = LEAST(rate_limit_counters.hits + 1, 2147483647)
     RETURNING hits`,
